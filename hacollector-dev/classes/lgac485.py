...
class LGACPacket:
    ...
    def set_packet_data(self, rawdata: bytes) -> bool:
        ...
        try:
            ...
            self.str_plasma = PAYLOAD_ON if (self.action & 0x10) or (self.fill_unknown2 & 0x10) else PAYLOAD_OFF
            self.error_code = self.fill_unknown4
            self.load_estimate = round(self.fill_model / 2.0, 1) if self.fill_model > 0 else 0.0
            ...
            if self.is_checksum_ok(possible_packet):
                ...
                if other_room:
                    ...
                    other_info = Aircon.Info(
                        ...
                        other_packet.str_plasma,
                        other_packet.error_code,
                        other_packet.load_estimate
                    )
